...
<ResponseField name="token_data" type="integer" required>
  ...
  ```json
  {
    "access_token": "<the access token to use in Benji Platform API calls",
    "refresh_token": "<the token to use when the access token expires",
    "access_token-expires_at": "<a datetime string value indicating when the access token will expire"
  }
  ```
</ResponseField>
