...
### open()
- This method opens the Benji Connect modal through the SDK. Typically you would attach this method to an onClick function in your member connect flow.
...
### onSuccess
Event that indicates a successful authentication between the member and a Benji loyalty rewards partner
...
- **metadata**: Metadata giving you information regarding the user details and user loyalty status to be able to immediately display or log in your system
```javascript
{
   "status": {
...
Note that the onExit will be called regardless of if the user has successfully linked their loyalty account. The only event that indicates a successful connection is the onSuccess event above.
</Expandable>

### onEvent
Event that indicates an action as part of the user flow on Benji Connect
...
### onError
Event that indicates an error has occurred as part of the Benji Connect flow
...
- **error**: The name of the error. The following error code names are supported:
   - **unexpected_error**: An unexpected error has occurred on the Benji Platform.
   - **partner_connect_error**: An error occurred while trying to connect the user with their selected loyalty network
