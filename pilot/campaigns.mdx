...
| **Trigger** | The trigger action associated with this campaign. | **Spend per Dollar** - for campaigns triggered by a user transaction <br />**Registration** - for campaigns triggered by by a user registration <br />**Customer Referral** - for a campaign triggered by a user referring another user to the partner <br />**Custom Action** - An action not supported natively by Benji. The partner will own the placement and rendering of the components for these action triggers |
